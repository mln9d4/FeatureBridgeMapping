...
train_dataset, test_dataset = create_splits(dataset_10_samples)
...
def save_model(model, model_config, beta_schedule):
    ...
    torch.save(save_temp, 'checkpoints/latest_10_sample_256_channels.pth')
    ...
    with wandb.init(project="diffusion_test", config=hyperparameters) as run:
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                val_loss = 0
                with torch.no_grad():
                    for batch in tqdm(test_loader, desc=f'Validation at Epoch {epoch+1}/{epochs}'):
                        X, y = batch['img_bev_embed'], batch['pts_bev_embed']
                        X = X.to(device)
                        y = y.to(device)
